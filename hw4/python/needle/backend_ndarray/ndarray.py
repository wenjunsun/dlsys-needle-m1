...
class NDArray:
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        # new_offset = 0 -- this is wrong because we can subindex another subindexed array
        new_offset = self._offset
        new_shape = []
        new_strides = []
        for i, idx_slice in enumerate(idxs):
        ...
        new_shape = tuple(new_shape)
        new_strides = tuple(new_strides)
        # NOTE: here need to return NDArray.make() in order to create a "new "array.
        # shouldn't return self because the __getitem__ gets a submatrix, but
        # this operation, like permute and other ones, shouldn't affect the original
        # array.
    ...
    def flip(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        # axes is a tuple of the dimensions to flip.
        ...
        return NDArray.make(list(self._shape), list(new_strides), self._device, self._handle, new_offset).compact()
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        new_shape = []
        slices = []
        for i, paddings in enumerate(axes):
            left_pad, right_pad = paddings
            new_shape.append(self.shape[i] + left_pad + right_pad)
            slices.append(slice(left_pad, left_pad + self.shape[i], 1))
        new_shape = tuple(new_shape)
        slices = tuple(slices)
        pad_array = self.device.full(new_shape, 0)
        pad_array[slices] = self # compact() done in __setitem__()
        return pad_array
        ### END YOUR SOLUTION
