    ...
    def __init__(
        ...
        **kwargs
        ...
        if isinstance(array, Tensor):
            ...
            # assert device is not None, "Tensor() should be called with Tensor(device = device)"
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    ### BEGIN YOUR SOLUTION
    for output_node in reverse_topo_order:
        if not output_node.requires_grad:
            continue
        # the follwoing can work too, but decided to use the existing helper function
        # out_grad = sum(node_to_output_grads_list[output_node])
        out_grad = sum_node_list(node_to_output_grads_list[output_node])
        output_node.grad = out_grad
        # then we calculate gradient's flow to this node's input nodes.
        if output_node.op is None:
            # this means that this node is just the input node, which doesn't have
            # any inputs or and op that generated this node. in this case
            # don't need to backpropagate any further. just return.
            continue
        input_grads = output_node.op.gradient(out_grad, output_node)
        if not isinstance(input_grads, tuple):
            input_grads = [input_grads]
        for (input_node, input_grad) in zip(output_node.inputs, input_grads):
