...
def epoch_general_cifar10(dataloader, model, loss_fn=nn.SoftmaxLoss(), opt=None):
    ...
    dataset_size = 0
    if opt:
        # training mode if optimizer is passed in
        model.train()
        for x, y in dataloader:
            batch_size = y.shape[0]
            # x = x.reshape((batch_size, np.prod(x.shape[1:])))
            ...
            # NOTE: loss itself is average loss, to get the loss over
            # entire batch need to multiply.
        ...
        # otherwise it is evaluation mode
        model.eval()
        for x, y in dataloader:
            # the follow is redundant because our data shouldn't have gradient enabled anyways when initialized.
            # x = x.data
            # y = y.data
            batch_size = y.shape[0]
            # x = x.reshape((batch_size, np.prod(x.shape[1:])))
