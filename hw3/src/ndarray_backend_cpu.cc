...
// TODO: equality between two floats...
void EwiseEq(const AlignedArray& a, const AlignedArray& b, AlignedArray* out) {
  for (size_t i = 0; i < a.size; i++) {
    if (a.ptr[i] == b.ptr[i]) { // comparing between two floats using == is not a good practice?
      out->ptr[i] = 1;
    } else {
      out->ptr[i] = 0;
    }
  ...
  for (uint32_t i = 0; i < m; i++) {
    for (uint32_t j = 0; j < p; j++) {
      ...
      for (uint32_t k = 0; k < n; k++) {
        sum_i_j += a.ptr[i * n + k] * b.ptr[k * p + j];
      }
      out->ptr[i * p + j] = sum_i_j;
    }
  }
  /// END YOUR SOLUTION
}
...
void MatmulTiled(const AlignedArray& a, const AlignedArray& b, AlignedArray* out, uint32_t m,
   ...
   */
  /// BEGIN YOUR SOLUTION
  // initialize all output matrix entries as zero.
  Fill(out, 0);
  
  for (uint32_t i = 0; i < m / TILE; i++) {
    for (uint32_t j = 0; j < p / TILE; j++) {
      for (uint32_t k = 0; k < n / TILE; k++) {
        const float* a_block_ptr = (const float*) &(a.ptr[i * n * TILE + k * TILE * TILE]);
        const float* b_block_ptr = (const float*) &(b.ptr[k * p * TILE + j * TILE * TILE]);
        float* out_block_ptr = (float*) &(out->ptr[i * p * TILE + j * TILE * TILE]);
        AlignedDot(a_block_ptr, b_block_ptr, out_block_ptr);
      }
    }
  }
  /// END YOUR SOLUTION
...
void ReduceMax(const AlignedArray& a, AlignedArray* out, size_t reduce_size) {
  ...
  /// BEGIN YOUR SOLUTION
  assert (out->size * reduce_size == a.size);
  for (size_t i = 0; i < out->size; i++) {
    size_t start_index = i * reduce_size;
    scalar_t curr_max = a.ptr[start_index]; // assume we at least have one element in axis we are reducing over.
    for (size_t j = 0; j < reduce_size; j++) {
      curr_max = std::max(a.ptr[start_index + j], curr_max);
    }
    out->ptr[i] = curr_max;
  }
  /// END YOUR SOLUTION
